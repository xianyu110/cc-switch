...
describe("MomoAI Provider Preset", () => {
  it("keeps the MomoAI gateway available without sponsored providers", () => {
    const momo = providerPresets.find((preset) => preset.name === "MomoAI");

    expect(momo).toBeDefined();
    expect(momo?.websiteUrl).toBe("https://momoai.asia/");
    expect((momo?.settingsConfig as any).env.ANTHROPIC_BASE_URL).toBe(
      "https://momoai.asia/v1",
    );
    expect(momo?.category).toBe("third_party");
  });
});
